...
def read_file(filename, debug_flag, homedir):


    df=pd.read_csv(f'{homedir}/datasource/{filename}')
    ...
    max_streak_date = ""
    streak_direction = ""
    first_quote_start_time_human_fmt = ""
    first_quote_start_time_human_fmt_local = ""
    last_quote_end_time_human_fmt = ""
    last_quote_end_time_human_fmt_local = ""
    
    lines_as_list = df.values.tolist()
    ...
    for quote in lines_as_list:
        ...
        last_quote_end_time_human_fmt =  strftime('%Y-%m-%d %H:%M:%S', gmtime(quote_end_time/1000))
        last_quote_end_time_human_fmt_local =  strftime('%H:%M:%S', localtime(quote_end_time/1000))
        ...
        if debug_flag:
            ...
            print("Quote End: " + last_quote_end_time_human_fmt + " UTC (" + last_quote_end_time_human_fmt_local + " GMT-3/Sao Paulo)")
    ...
    if debug_flag:
        ...
        print("          to: " + last_quote_end_time_human_fmt + " UTC (" + last_quote_end_time_human_fmt_local + " GMT-3/Sao Paulo)")
        print(" ")
    ...
    with open(f'{homedir}/output/output.txt', "a") as output_file:
        output_line = f'filename, first_quote_start_time, first_quote_start_time_local, last_quote_end_time, last_quote_end_time_local, max_streak, max_streak_date, streak_direction'
        output_file.write(output_line + "\n")
        output_line = f'\'{filename}\', \'{first_quote_start_time_human_fmt}\', \'{first_quote_start_time_human_fmt_local}\' , \'{last_quote_end_time_human_fmt}\', \'{last_quote_end_time_human_fmt_local}\' , {max_streak}, \'{max_streak_date}\', \'{streak_direction}\''
        output_file.write(output_line + "\n")
        output_file.close()

def list_files_in_dir(homedir):
    return subprocess.run(["ls", "--ignore=bkp*", f'{homedir}/datasource'], capture_output=True, text=True).stdout.split()
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Usage: python3 analyzedatasource.py [debug] [homedir]")
        sys.exit(1) 
 
    
    for file in list_files_in_dir(homedir):
        print("==============================================================================================")
        print("Processing file: " + file)
        print("----------------------------------------------------------------------------------------------")
        print(" ")
        read_file(file, debug_flag, homedir)